# PURE Siegel — Authenticity Standard v1.0 (DRAFT)

> **Document version:** 1.0 — **DRAFT for review** (not in force)
> **In force:** no
> **Status:** draft. Not yet binding on any certification. This document is PREPARED for the PS-GOV-04 30-day public review; it has **not** been adopted. Adoption is a governance act (version bump + 30-day public consultation) and has not occurred.
> **Supersedes (when adopted):** v0.1, v0.2 and the v0.3 draft. Certifications already issued under v0.1/v0.2 remain bound to the version they were issued under and are **not** re-tiered.
> **Canonical URL (on adoption):** `/standard/v1.0` (and via API `GET /api/standard/current`)
...
> **Diff:** on adoption, a line-level delta from v0.2 is published at `GET /api/standard/v1.0/diff/v0.2`.
>
...
> and "changes from v0.2": that narrative is the accurate lineage of the tier model v1.0
> adopts and is preserved verbatim rather than rewritten.
>
> **DRAFT ADDITIONS (2026-09-14) — pending PS-GOV-04 review, NOT yet adopted.** The full
> v1.0 packaging (`memory/guetezeichen-packaging-concept.md`) calls for a
> **named-signatories block (G2)**, a **normative interpretive-principles section §1.1
> (G3)**, and an **in-period surveillance obligation (G5)**. Following the founder's
> decisions of 2026-09-14 (`memory/v1.0-open-decisions.md`), all three are now **drafted in
> this document**: G2 as the "Responsible for this Standard" block at the end; G3 as
> §1.1; G5 as §9 (*In-period surveillance*). This is **draft normative text pending the
> PS-GOV-04 process** (Fachbeirat-Stellungnahme + 30-day public consultation + ≥30-day lead
> time, non-retroactive) and is **not in force** until v1.0 is adopted. The two G2 role
> titles are now filled with the founder's decided titles (Patrick Jaritz — Technical Lead;
> Silvia Wirnsberger — Managing Director). Note also that
> §7/§10 still reference "Mark Usage License v0.1" verbatim from the v0.3 draft (a
> pre-existing reference), and the Licence's own tier/assurance wording is reconciled to
> the three-tier model at the Licence version bump — see `docs/license-v1.0.md` §3.4.
...
**What changed in v0.3 (summary; full list in §12).** v0.2 certified two
...
> **DRAFT (new in v1.0, 2026-09-14) — pending PS-GOV-04 review; not in force.**

### 1.1 · Principles of interpretation
Where this Standard is ambiguous, it is read in favour of *disclosure over concealment*, *the reader's ability to verify over the applicant's convenience*, and *the narrow reading of what the seal claims*. The Standard does not police literary quality, factual accuracy or editorial choice.

*(The third sentence above — that the Standard polices neither literary quality nor
factual accuracy nor editorial choices — already exists as prose in §1. §1.1 does not
change §1; it **promotes** that limit, together with the two interpretive maxims, from
prose to a **citable interpretive rule** for borderline cases and reviewer calibration.)*
...
(§7), and any false field is a basis for revocation (§10).
...
scope for this draft** and are tracked as an open item (§13); they will be specified once the medium list
...
| `declared_category` | Badge (mark) | One-line meaning (DE) | One-line meaning (EN) |
|---------------------|--------------|------------------------|------------------------|
| `human_authored` | **AUCTOR** | Ohne KI erstellt. | Created without AI. |
| `human_assisted` | **AUXITOR** | KI-unterstützt, nicht KI-erzeugt; offengelegt. | AI-assisted, not AI-generated; disclosed. |
| `human_led` | **DUCTOR** | Menschlich geführt, KI offengelegt. | Human-led, AI disclosed. |

The badge is a single language-neutral **mark** (AUCTOR / AUXITOR / DUCTOR), read
identically in every language and shown in both the German and English UI, with the
localized one-line meaning beneath it. The mark is never shown alone — the meaning
subtitle always accompanies it (the AOC/DOC pattern).

**The display marks are AUCTOR / AUXITOR / DUCTOR; the machine values are
unchanged.** `declared_category` stays `human_authored` / `human_assisted` /
`human_led` in the signed payload, the ledger and the API — this is display only.
This supersedes the §8.1 "German-only coined mark" decision below, per
`docs/trademark-tier-names-shortlist.md` §"FRONT-RUNNER (decided direction
2026-09-16)". The change remains gated on the standing gates (native-speaker
connotation check AT/DE/CH, EUIPO/ÖPA/DPMA clearance, English sayability) and must
be settled before certificate #1.
...
## 9. In-period surveillance — ongoing reliability confirmation *(DRAFT, new in v1.0)*

> **DRAFT (2026-09-14) — pending PS-GOV-04 review; not in force.** This is a new
> obligation on the holder and therefore a Standard change: it is introduced under
> PS-GOV-04 (30-day public consultation + ≥30-day lead time, non-retroactive) and takes
> effect only when v1.0 is adopted.

Certificates issued by PURE **do not expire.** This Standard introduces **no** certificate
term and **no** `expires_at` / `valid_until` concept — a fixed expiry would have no
epistemic basis (`memory/recertification-concept.md` §2). Ongoing reliability is instead
confirmed periodically by the holder.

**Cadence.** Every **two years (biennial)**, counted from the certificate's issuance date,
the holder is asked to re-confirm that the certified work is
unchanged. There is no certificate term against which this is measured; the clock runs
from issuance only.

**What the confirmation is — and is not.** The confirmation is a **re-attestation by the
holder**, not a server-side verification of the work. PURE never receives the work itself;
it stores only `sha256(content_text)` of the canonical content and holds **no bytes of the
work to re-hash on its own.** Accordingly:

- The holder periodically **re-confirms**, as an attested statement, that the certified
  work is unchanged.
- **Where the holder re-supplies the canonical content** at confirmation time, the registry
  can compare its hash against the stored `content_sha256` and **flag a divergence.** This
  hash comparison is available only when the holder supplies the content; it is **not** a
  standing machine check the registry can run unilaterally, and the registry does **not**
  claim to machine-verify that the work is unchanged.
- A hash mismatch is **not misconduct and not a failed confirmation.** A changed work is a
  normal event; it is a (voluntary) **recertification trigger** — the work would be judged
  afresh under the then-current Standard.

**Ledger and public page.** A completed confirmation appends a **`surveillance` event to
the append-only ledger.** It creates no new status and changes no existing one. The public
verification page `/v/<cert_id>` publishes the **date of the last confirmation** — or, when
a confirmation is overdue, *"confirmation overdue since …"* — as a **published fact only.**
There is **no automatic status change and no automatic suspension** on non-response.

**Out of scope for v1.0.** Any sanction for non-response (e.g. suspension) is deliberately
**not** part of this obligation. A sanction is governed by PS-GOV-05 together with the
PS-GOV-02 hearing right and must be decided separately with the Fachbeirat.

<!-- DRAFT NOTE: a stronger, genuinely third-party-verifiable work fingerprint — so that
     surveillance could become server-verifiable rather than a holder re-attestation — is a
     FUTURE enhancement, tracked in memory/BACKLOG.md. It is NOT part of v1.0. -->
...
## 13. Open items (must close before adoption)
...
- **Pilot:** confirm the §4.1 computed tier matches reviewer judgement across at
  ...
  *(Amended 2026-08-11. This item originally read "5–10 real, already-decided
  ...
  is the safeguard.)*
- **In-period surveillance (§9) — obligation specified, feature not yet built.** The
  biennial re-attestation duty is defined in §9, but the holder-facing portal action, the
  `surveillance` ledger event and the `/v/` last-confirmation line do **not** yet exist
  (tracked in `memory/BACKLOG.md`). The first confirmation only falls due ~2 years after
  certificate #1, so this does **not** block adoption of the text — but it must be built
  before the first confirmation is due.
...
> **DRAFT (2026-09-14) — pending PS-GOV-04 review; not in force.**

<!-- Names and roles are the fields that belong in the hash; portraits and bios stay
     OUTSIDE the hashed document (rendered from a web constant, cf.
     frontend/src/pages/Fachbeirat.jsx), so a new photo never invalidates a certificate. -->

| | |
|---|---|
| **Technical responsibility** | Patrick Jaritz, Technical Lead |
| **Legal responsibility** | Silvia Wirnsberger, Managing Director |
| **Release** | Management, PURE Media Certification GmbH |

The Fachbeirat must be consulted before the public review per PS-GOV-04 § 5; the
minutes are published at `/governance`.
